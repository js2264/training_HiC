...
## Create a `conda` environment and install many required softwares for HiC & co.

To process Hi-C reads, we need at least: 

- An aligner (e.g. `bowtie2`)
- An pairs processor (e.g. `hicstuff`)
- An Hi-C contact matrix generator (e.g. `cooler`)

To install these softwares, we will rely on a dedicated `conda` environment.
Let's see how we can install `conda`, create en environment and install required packages. 

```{sh filename="bash"}
curl micro.mamba.pm/install.sh | bash
#  ------------------------------ - ----
# `curl ...`: Fetch a file from internet
#                                `|`: ... and ...
#                                  `bash`: Execute it
```

At this stage, you will have to restart a new terminal to continue the work. 
Otherwise, you may encounters errors such as `command not found`.
...
micromamba create -n HiC_micromamba \
   'python>=3.7.12' \
...
#  ---------- ------ ------ --------------
# `micromamba`: run micromamba main command
#            `create`: Create a new conda environment ...
#                   `-n HiC`: ... named "HiC" ...
#                          `python=`: ... and pre-install `python` package in this "HiC" env.

micromamba activate HiC_micromamba
#  ----- -------- ---
# `micromamba`: run micromamba main command
#       `activate`: Activate an existing environment ...
#                `HiC`: ... specifically the "HiC" env. we created
...
::: {.callout-tip}
## conda vs `anaconda` vs `miniconda` vs `mamba` vs `micromamba`
All these softwares fit the same purpose: managing package installation in dedicated environments. 
`conda` is the actual **package manager**, while `anaconda` or `miniconda` are 
software distributions, i.e. a set of softwares, **including `conda`**. 
`anaconda` includes 150+ scientific softwares whereas `microconda` has a much 
tinier disk storage footprint. It includes `python` by default, but not much else. 
`mamba` and `micromamba` are a newer flavour of `conda` and `miniconda`: they 
can perform the same types of operations (create environment, install packages, ...)
but much faster as it supports parallelized jobs!
:::
...
* --matfmt=STR         The format of the output sparse matrix. Can be "bg2" for 2D Bedgraph format, "cool" for Mirnylab's cooler software, or "graal" for graal-compatible plain text COO format. [default: graal]
* --no-cleanup         **Super important!!** Needed to keep intermediate pairs file
...
- IMO, `--filter` is what makes `hicstuff` unique compared to other workflows. **DO USE IT!!**
- `--matfmt cool` should be default in hicstuff. IMO, other formats are not as convenient, especially if you are going to map on larger genomes. 
...
A number of `.pairs` files are present in `results/tmp/`. 
...
```{sh}
-rw-rw-r-- 1 rsg rsg 615M 27 janv.  15:54 results/tmp/CH112.valid_idx_pcrfree.pairs
...
- The `*_pcrfree.pairs` file is probably the one you want to use in your analyses. If you check the output of `hicstuff`, you can see that it is indeed generated **after** pairs filtering, so it's ok to use it!
...
- The remaining `*.valid.pairs` is redundant with the `*.valid_idx.pairs` and is probably not useful, it can be deleted. 
:::

A good practice is also to remove `.bam` files kept in the `tmp/` folder. 

Eventually, I recommend creating a new `pairs/` folder, moving your `.pairs` files there, and deleting the remaining content of the `tmp/` folder: 

```{sh filename="bash"}
mkdir results/pairs/
mv results/tmp/*valid_idx*.pairs results/pairs/
...
> Understanding the `distance_law` plot

![](results/plots/CH112_distance_law.pdf){height=500 width=100%}

The distance law is a good way to check that each of the chromosomes behaves as expected. In a yeast G1-synchronized sample, you expect a straight-ish line, with a slope of ~ -1.2 -> 1.5. In a yeast G2M-synchronized sample, you expect increased contacts in the 10-50kb range, visible as a "shoulder" in the curve. 
...
You probably have noticed that `hicstuff pipeline` automatically creates a `results/<PREFIX>.cool` file. 
To understand the differences between `results/<PREFIX>.cool` and `results/matrix/<PREFIX>.cool`, one 
can use `cooler info`. Have a look at the `"bin-size"` and the `"bin-type"` keys printed in the terminal. 

```{sh filename="bash"}
cooler info results/CH112.cool
cooler info results/matrix/CH112.cool
```

The immediate output of `hicstuff pipeline` is a bin-less contact matrix! Each column/row actually represents 
...
Here, we use `cooler cload` to convert a `.pairs` file into a `.cool` file. We could also have used `hicstuff rebin`, 
which uses a slightly different syntax to perform the same operation. 
:::
...
PREFIX=CH112
BINSIZE=1000
MULTIRES="1000,2000,4000,8000,16000"
THREADS=12
ENZYME="DpnII,HinfI"
GENOME="genomes/S288c/S288c.fa"
...
cooler cload \
   ...
   --chrom1 2 --pos1 3 \
   --chrom2 4 --pos2 5 \
   <(sed 1d results/"${PREFIX}".chr.tsv | cut -f1-2):"${BINSIZE}" \
   results/pairs/"${PREFIX}".valid_idx_pcrfree.pairs \
   results/matrix/"${PREFIX}".cool

# ------ cool -> mcool
cooler zoomify \
   --nproc "${THREADS}" \
   --resolutions "${MULTIRES}" \
   --balance \
   --out results/matrix/"${PREFIX}".mcool \
   results/matrix/"${PREFIX}".cool
...
chromosight detect \
   --pattern loops \
   --threads "${THREADS}" \
   results/matrix/"${PREFIX}".mcool::/resolutions/"${BINSIZE}" \
   results/loops/"${PREFIX}"
```
